...
const findLockFile = (packageManager: PackageManagerInfo) => {
  ...
  // Avoid reading and allocating memory for all directory entries in workspace via fs.readdirSync(workspace).
  // Checking existence directly via fs.existsSync(path.join(workspace, item)) is O(1) per lockfile pattern.
