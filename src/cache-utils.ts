...
export const getCommandOutput = (
  ...
  if (cached) {
    core.debug(`[Bolt] Using cached output for command: ${toolCommand}`);
    return cached;
  }
  ...
  commandOutputCache.set(cacheKey, resPromise);

  // Remove from cache if the promise fails to avoid caching errors
  resPromise.catch(() => {
